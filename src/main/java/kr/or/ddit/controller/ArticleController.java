...
public class ArticleController {
    ...
    // URL 요청 접수
    // 뷰(등록페이지) 페이지를 보여 주기 위해 newArticleForm() 메서드를 추가
    ...
    // new.mustache 에서 form 태그에 action 속성에 맵핑된 'articles/create'로 설정했기 때문에 맵핑 될 수 있다.
    @PostMapping("/articles/create")
    public String createArticle(ArticleForm form){
        // ArticleForm 타입의 form 객체를 매개변수로 선언합니다.
        // 폼에서 전송한 데이터기 dto에 잘 담겼는지 확인하기 위해서 출력문을 추가함.

        // 로깅을 사용하기 위해 클래스 명 위에 @Slf4j 어노테이션을 사용합니다.
        // dto에 입력한 데이터가 정상적으로 바인딩되어 넘어오기 위해서는 mustache 파일에 작성된 form 태그 내에
        // 입력 태그들의 name과 ArticleForm 클래스 내 필드명이 갈아야만 값이 정삭적으로 바인딩되어 들어올 수 있습니다.
        ...
        // createArticle->form : ArticleForm{title='제목입니다01', content='내용입니다01'}
        ...
        // entity로 변환된 Article에 id가 null인지로 확인할 수 있고, 데이터가 저장 후의 Article의 id가 자동 증가된 값으로 1이 된 걸
    ...
    public String show(@PathVariable("id") Long id, Model model){
        //@PathVariable 어노테이션은 URL에 포함되어 있는 즉, 요청 경로에 포함되어 들어오는 경로 파라미터의 값을 가져올 때 사용합니다.
        // 이때, '{}'에 설정된 파라미터가 키 값이 해당 메소드의 파라미터 변수와 동일해야만 데이터를 바인딩 할 수 있습니다.
        ...
        // 우리가 활용할 데이터의 규격은 Article 객체를 반환할 줄, 상세보기 페이지는 Model에 담아 전달하고자 한다.
        // 그렇기 때문에, Optional 타입을 활용하여 넘기지 않고 Article과 같은 타입으로 반환할 수 있도록 강제 변환이 필요하다.
        // orElse 활용
        // orElse(null) : id 값으로 데이터를 찾을 때 해당 id와 일치하는 게시글 데이터가 없으면 null을 반환하고,
        //                값이 있으면 articleEntity 변수에 Article 객체 데이터를 담아 반환한다.
    ...
    public String index(Model model){ // index() 메소드의 매개변수로 Model 객체를 받아옴(목록 데이터 전달 위함)
        // 캐7스팅이란 형변환이라고도 하며 데이터 타입을 변환하는 것을 말합니다.
        // 자바에서 상속 관계가 있는 특정 객체는 상황에 따라 더 넓은 범위도 해석될 수도 있고, 때때로 적은 범위로 해석될 수도 있습니다.
        // 이때, 넓은 범위로 형변환하는 것을 업캐스팅(Upcasting), 좁은 범위로 형변환하는 것을 다운 캐스팅(DownCasting)이라고 합니다.
        // 이를 들의 과이을 활용을 하셨다면 업캐스팅이고, 실행에서 다시 좁을로 형변환하셨다면 다운캐스팅입니다.

        // 1. 모든 데이터 가져오기
        // findAll() 메소드는 원래 반환 타입이 Iterable<> 이지만, ArrayList로 재정의 했기 때문에, 다운 캐스팅입니다.
        // Iterable<Article> articleEntity = articleRepository.findAll(); 은 업캐스팅으로 데이터 반환
        // Iterable(I) <- Collection(I) <- List(I) <- ArrayList(C)
        ArrayList<Article> articleArrayList= articleRepository.findAll();
    ...
    public String edit(@PathVariable("id") Long id, Model model) {
        ...
        // 만약 데이터를 찾지 못하면 null를 반환하고, 데이터를 찾았다면 Article 타입의 articleEntity로 작성됩니다.
        Article articleEntity = articleRepository.findById(id).orElse(null);
    ...
    // URL 요청 접수
    // 매개변수로 dto 받아오기
    // 수정의 기능이라면 HTTP 메서드 방식의 PATCH를 사용해야 하지만, form 태그가 지원하느 메소드 방식은
    ...
    public String update(ArticleForm form){
        ...
        // db에서 id로 일치하는 게시글 정보를 Repository의 findById() 메소드를 이용해 가져와 Article 타입의 객체로 저장
        // 데이터가 없다면 null를 반환한다.
    ...
    public String delete(@PathVariable("id") Long id, RedirectAttributes ra){
        ...
        if(target !=null){
            ...
            // 일회성 메세지를 전달하기 위한 설정
            // RedirectAttributes 객체는 객체명에서도 힌트를 얻을 수 있듯이 페이지 이동방식이 리다이렉트 인경우 일회성 데이터를
            // 전달할 수 있습니다.
