...
import org.springframework.ui.Model; // ✅ import 누락 주의!
import org.springframework.web.bind.annotation.GetMapping;

// 1. 어노테이션(annotaion)이란?
// 소스 코드에 추가해 사용하는 메타 데이터의 일종입니다.
//메타 데이터는 프로그램에서 처리해야 할 데이터가 아니라 컴파일 및 실행 과정에서 코드를 어떻게 처리해야 할지
// 알려주는 추가 정보입니다.
// 자바에서는 어노테이션 작성 시, 앞에 '@'기호를 붙여 사용합니다.

// 컨트롤 선언과 동시에 자동으로 임포트
// 컨트롤러 선언 : 해당 파일이 컨트롤러임을 선언합니다.
...
public class FirstController {
    ...
    public String niceToMeetYou(Model model) {
        ...
        model.addAttribute("username", "홍길동"); // ✅ 올바른 문법

        // greetings.mustache 파일 반환
        // greetings 문자열을 입력하게 되면 해당 문자열을 서버가 알아서 templates 디렉토리 내에서
        // 같은 이름의 mustache 파일을 팢아 웹 브라우저로 전송합니다. (템플릿 엔진으로 mustache를 활용
    ...
    //      어때, 해당 요청은 Get 방식의 Method 방식으로 들어온 요청이기 때문에 Get방식의 요청을 받기
    ...
    public String seeYouNext(Model model) {
        ...
        // 의 {{nickname}}자리의 변수값으로 값을 받을 수 있다.
        model.addAttribute("nickname", "메뚜기"); // ✅ 올바른 문법
        // 반환값은 요청에 따라 보여 줄 뷰 템플릿 페이지를 적습니다.
        return "goodbye";
    }
}
