...
public class FirstApiController {
    ...
    /*
        ...
        서버는 이리한 모든 클라이언트 요청에 응답해야 합니다. 웹 브라우저뿐만 아니라 어떤 기기와도 기기에 맞는 뷰 페이지
        ...
        REST API를 개발하다보면 HTTP Method 방식 설정이 필요합니다. 이때, GET,POST,PUT,PATCH,DELETE 등 다양한 Method 방식을
        선택하는데, Method 방식 중 PUT과 PATCH는 차이점이 존재합니다.
        
        # PUT과 PATCH의 차이점
        - PUT : 기존 데이터를 전부 새 내용으로 변경합니다. 만약 기존 데이터가 없다면 새로 생성
        - PATCH : 기존 데이터 중에서 일부면 새 내용으로 변경합니다.
