...
// 식당을 예로 들면, 웨이터에게 주문이 들어오면 이를 전달받은 주방장이 요라를 총괄합니다.
// 재료가 필요하면 보조 요리사가 이를 가져오고, 여기서 웨이터는 컨트롤러, 주방장은 서비스, 보조 요리사는
// Repository로 볼 수 있다.
// 손님(클라이언트)이 음식을 주문하면 웨이터(컨트롤러)가 이를 받아 주방장(서비스)에게 전달하고,
// 주방장(서비스)은 정해진 레시피에 따라 요리를 한다.
...
// 그리고 서버가 runtime시, 객체화 되어 객체 주입 시 활용할 수 있다.
...
public class ArticleService {
    ...
    public Article update(Long id, ArticleForm dto) {
        ...
        // 클라이언트에서 받은 수정 데이터가 담긴 dto를 db에서 활용할수 있도록 엔티티로 변환해
        // article 변수에 저장한다.
        Article article=dto.toEntity();
        // 중간에 실행이 잘 되는지 확인하기 위해서 id와 article의 내용을 로그로 출력합니다.
        log.info("id : {}, article : {}",id,article.toString());

        // 2. 타깃 조호ㅚ하시 (db에 대상 엔티티가 있는지 조회)
        // db에서 대상 엔티티를 조회해 가져온다.
        // .findByUd(id) 메소드를 통해서 id에 해당하는 엔티티를 가져오되 없다면 null을 반환한다.
        Article target=articleRepository.findById(id).orElse(null);

        // 3. 잘못된 요청 처리하기(대상 엔티티가 없거나 수정하려는 id가 잘못됐을 경우 처리)
        // - 요청 경로로 들어온 id와 일치하는 Article 정보가 없는 경우도 에러이므로 null 체킹
        // - 요청 경로로 들어온 id와 수정 데이터로 넘어온 id로 얻어온 Article정보의 id가 불일치하는
        //   경우, 수정하고자 하는 데이터의 정보가 다르므로 에러
        if(target==null || id !=article.getId()){
            log.info("update->잘못된 요청! id {}, article:{}",id,article.toString());
            // ResponseEntity 반환
            // ResponseEntity의 상태(status)에는 400 또는 HttpStatus.BAD_REQUSET를 설정하고
        ...
        //     "title" : "게시글 수정 111",
        // }
        // 그렇기 때문에 단일 수정 시 수정하지 않은 항목은 유지하면서 나머지 항목만 수정되도록 설정이 필요합니다.
        // id를 통해 조회한 기존 데이터에 해당하는 target 정보를 수정하기 위해 던진 article
        // 데이터로 데이터가 있는 항목에 대해서만 수정 후, 수정하기 위해서 repository로 target을 던지면
        // 단일 데이터 수정이 가능합니다.

        // 기존 데이터에서 새롭게 수정된 데이터를 합쳐놓은게 target 이므로
    ...
    public Article delete(Long id) {
        ...
        // db에 삭제할 대상 엔티티가 있는지 조회하고 없으면 null을 반환한다.
        Article target=articleRepository.findById(id).orElse(null);
