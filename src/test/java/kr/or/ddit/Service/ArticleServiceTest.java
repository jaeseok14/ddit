...
class ArticleServiceTest {
    ...
    // # 테스트 결과 처리
    // 테스트를 통과하면 지속적인 리팩토링으로 코드를 개선할 수 있고, 통과하지 못하면 디버깅을 통해 잘못된 부분을 수정할 수 있습니다.
    //
    ...
    // - assertNotEquals(expected,actual) : 두 값이 같지 않을 때 검증
    ...
    void index() {
        ...
        // 리스트 데이터가 들어옵니다.
        // 예상 데이터 객체로 저장
        Article a=new Article(1L,"개똥이의 하루","즐거운 여행1!");
        Article b=new Article(2L,"개똥이의 하루","즐거운 여행2!");
        Article c=new Article(3L,"개똥이의 하루","즐거운 여행3!");

        // Arrays.asList() 메서드로 합친 정적 리스트를 새 ArrayList로 만들어 expected에 저장합니다.
        // Arrays.asList() 메소드는 입력된 배열 또는 2개  이상의 동일한 타입의 데이터를 정적 리스트로 만들어
        // 반환합니다. 그렇기 때문에 고정적인 크기이고 add(),remove()와 같은 함수를 사용할 수 없다.
        List<Article> expected=new ArrayList<>(Arrays.asList(a,b,c));

        // 2. 실제 데이터
        // 실제 데이터는 articeService.index() 메소드를 호출해 그 결과를 List<Article> 타입의
        // articles에 받아옵니다.
    ...
    void show_success() {
        ...
        // 1. 예상 데이터
        Article expected = new Article(id, "개똥이의 하루", "즐거운 여행1!");
    ...
    void show_success2() {
        ...
        Long id =-1L;
        //1. 예상 데이터
        // 예상 데이터는 존재하지 않는 id안 -1을 조회한다고 가정
        // 해당 경우 db에서 조화되는 내용이 없어 null을 반환할 것이므로 expected 객체에 null을 저장한다.
    ...
    void show_failed() {
        ...
        Article expected = new Article(id, "개똥이의 하루", "즐거운 여행");
    ...
    void create_failed() {
        ...
        Long id = 4L;
        String title = "개똥이의 여름4";
        String content = "즐거운 여행4";
        // 위 3가지 항목으로 만든 Article 정보는 id를 포함된 데이터이나, 존재하지 않는 4번째 게시글 정보로
        // 등록 기능이 아닌 수정 가능을 요청했기 때문에 없는 데이터 null이 반환됩니다.
        // 그렇기 때문에 예상 데이터는 null
        Article expected = null;
